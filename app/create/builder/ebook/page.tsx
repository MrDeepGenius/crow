// ============================================
// EBOOK STUDIO - Producto editorial premium con IA
// ============================================
// Ruta: /create/builder/ebook
...
const VERSIONS_KEY = "crow_ebook_versions";
...
export default function PdfBuilderPage() {
  ...
  const [engine] = useState(() => new PdfGenerationEngine("ebook"));
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const savedIdea = sessionStorage.getItem("crow_ebook_idea");
    setIdea(savedIdea);
    try {
      const raw = window.localStorage.getItem(EBOOK_PRODUCT_KEY);
      if (raw) {
    ...
    try {
      const b = sessionStorage.getItem("crow_ebook_blueprint");
      const s = sessionStorage.getItem("crow_ebook_spec");
  ...
  const persistProduct = (next: PdfProduct): void => {
    ...
    if (typeof window !== "undefined") {
      try {
        window.localStorage.setItem(EBOOK_PRODUCT_KEY, JSON.stringify(next));
      } catch {
  ...
  const approveBlueprint = async (): Promise<void> => {
    ...
    try {
      if (typeof window !== "undefined") {
        sessionStorage.setItem("crow_ebook_blueprint", JSON.stringify(blueprintDraft));
        sessionStorage.setItem("crow_ebook_spec", JSON.stringify(specDraft));
      }
  ...
  if (!idea) {
    return (
      <main style={{ minHeight: "100vh", background: "#050505", color: "#fff", fontFamily: FONT, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px" }}>
        ...
        <h1 style={{ fontSize: "26px", margin: "0 0 12px" }}>No hay un ebook para crear.</h1>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "16px", flexWrap: "wrap", marginBottom: "20px" }}>
              <div>
                <div style={{ color: "#a855f7", fontSize: "12px", fontWeight: "bold", letterSpacing: "1px" }}>
                  EBOOK · {product.editorialStatus.toUpperCase()} · v{product.version}
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                ...
                <button onClick={() => void downloadPdf()} disabled={working} style={{ ...viewBtn(true), opacity: working ? 0.6 : 1 }}>
                  {working ? "Trabajando..." : "Exportar PDF"}
                </button>
              </div>
            </div>
          ...
          <div ref={finalRef}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", flexWrap: "wrap", marginBottom: "16px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                ...
                <button onClick={() => void downloadPdf()} disabled={working} style={viewBtn(true)}>Exportar PDF</button>
              </div>
            </div>
            {pdfUrl && !embedFailed ? (
              <iframe
...
function BlueprintApproval({
  ...
  return (
    <div style={{ background: "rgba(15,15,18,0.9)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "20px", padding: "30px", marginBottom: "24px" }}>
      ...
      <h1 style={{ fontSize: "30px", margin: "8px 0 6px" }}>Tu blueprint está listo</h1>
      <h2 style={{ fontSize: "20px", margin: "0 0 6px" }}>{blueprint.title}</h2>
      ...
      <p style={{ color: "#aaa", fontSize: "14px", margin: "0 0 20px", lineHeight: 1.6 }}>
        <strong style={{ color: "#fff" }}>Promesa:</strong> {blueprint.brief.promise}
      </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "12px", marginBottom: "20px" }}>
            {blueprint.chapters.map((ch, i) => (
              <div key={ch.id} style={{ padding: "16px", borderRadius: "12px", background: "#0c0c0f", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div style={{ color: "#555", fontSize: "11px", marginTop: "6px" }}>{ch.sectionTitles.join(" · ")}</div>
              </div>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "12px", marginBottom: "20px", fontSize: "13px", color: "#aaa" }}>
            <div><strong style={{ color: "#fff" }}>Público:</strong> {blueprint.targetAudience}</div>
            <div><strong style={{ color: "#fff" }}>Tono:</strong> {spec.tone} · <strong style={{ color: "#fff" }}>Estilo:</strong> {spec.style}</div>
            <div><strong style={{ color: "#fff" }}>Recursos:</strong> {blueprint.brief.resourcesNeeded.slice(0, 2).join("; ")}</div>
          </div>
