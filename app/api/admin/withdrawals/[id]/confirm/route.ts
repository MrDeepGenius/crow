import { NextRequest, NextResponse } from "next/server";
import { isAdminRequest } from "@/app/services/auth/auth";
import { confirmWithdrawal } from "@/app/services/withdrawals/withdrawalService";

export const dynamic = "force-dynamic";

// POST /api/admin/withdrawals/[id]/confirm — confirmar retiro con TX Hash
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const admin = isAdminRequest(req);
  if (!admin.ok) {
    return NextResponse.json({ ok: false, error: admin.reason }, { status: admin.reason === "UNAUTHENTICATED" ? 401 : 403 });
  }

  const { id } = await params;
  const body = await req.json().catch(() => ({}));
  const { txHash } = body as { txHash?: string };

  if (!txHash) {
    return NextResponse.json({ ok: false, message: "TX Hash requerido." }, { status: 400 });
  }

  const actor = admin.user?.id ?? "admin";
  const result = confirmWithdrawal(id, txHash, actor);

  if (!result.ok) return NextResponse.json({ ok: false, message: result.message }, { status: 400 });
  return NextResponse.json({ ok: true, message: result.message });
}
