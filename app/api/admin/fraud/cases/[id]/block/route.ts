// POST /api/admin/fraud/cases/[id]/block — atajo BLOCK_COMMISSION
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    try {
      const updated = reviewCase(id, "BLOCK_COMMISSION", adminId, typeof body.note === "string" ? body.note.slice(0, 500) : "");
